...
class AEPBRTIncludeGeometryTemplate(BaseTemplate):
    ...
    def newFileDialog(self, attr, label):
        ...
        #print( "\tvalue : %s" % value )
        self.pbrtfileCtrl = cmds.textFieldButtonGrp(label=label, text=value,
            buttonLabel="Open", buttonCommand="browseFiles")
        cmds.textFieldButtonGrp(self.pbrtfileCtrl, e=1, 
            buttonCommand=lambda: self.getFilePath(self.pbrtfileCtrl, nodeName, attrName),
    ...
    def replaceFileDialog(self, attr, label):
        ...
        if attrName == 'pbrtfile':
            if existingValue is None:
                existingValue = ""
            #print( "\tsetting text field group : %s" % existingValue )

            cmds.textFieldButtonGrp( self.pbrtfileCtrl, e=1, label=label,
                text=existingValue,
                buttonLabel="Open", buttonCommand="browseFiles" )
            cmds.textFieldButtonGrp(self.pbrtfileCtrl, e=1, 
                buttonCommand=lambda: self.getFilePath(self.pbrtfileCtrl, nodeName, attrName),
    ...
    def newpbrtfileDialog(self, attr):
        self.newFileDialog(attr, "")

    def replacepbrtfileDialog(self, attr):
        self.replaceFileDialog(attr, "PBRT File")

    def buildBody(self, nodeName):
        #print( "ae template build body : %s" % nodeName )
        #self.beginLayout("params", collapse=0)
        self.callCustom(self.newpbrtfileDialog, self.replacepbrtfileDialog, "pbrtfile")
        #self.endLayout()
