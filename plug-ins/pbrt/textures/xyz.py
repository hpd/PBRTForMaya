...
kPluginNodeName = "PBRTxyz"
kPluginNodeClassify = "utility/color"
kPluginNodeId = OpenMaya.MTypeId(0x87068)

class xyz(OpenMayaMPx.MPxNode):
    def __init__(self):
    ...
    def compute(self, plug, block):
        if plug == xyz.mOutColor:
            resultColor = OpenMaya.MFloatVector(0.0,0.0,0.0)
            
            outColorHandle = block.outputValue( xyz.mOutColor )
...
def nodeInitializer():
    ...
    try:
        xyz.mPreset = eAttr.create("preset", "pre" )
        eAttr.setKeyable(1) 
        eAttr.setStorable(1)
        eAttr.setReadable(1)
        eAttr.setWritable(1)
        ...
        xyz.mValue = nAttr.create("value", "val", OpenMaya.MFnNumericData.k3Float)
        ...
        nAttr.setDefault(0.95047,1.0,1.08883)

        xyz.mOutColor = nAttr.createColor("outColor", "oc")
        nAttr.setStorable(0)
        nAttr.setHidden(0)
        nAttr.setReadable(1)
        nAttr.setWritable(0)
    ...
    try:
        xyz.addAttribute(xyz.mPreset)
        xyz.addAttribute(xyz.mValue)

        xyz.addAttribute(xyz.mOutColor)
    except:
        sys.stderr.write("Failed to add attributes\n")
        raise

    try:
        xyz.attributeAffects (xyz.mValue, xyz.mOutColor)
    except:
